...
class MapProvider extends Component {
  state = {
    ...
    counties: [
      ...
      {value: 'Gümüşhane', label: 'Günüşhane'},
  ...
  render() {
    ...
    const {currentMap, waitingSchoolCount, counties} = this.state
    const {setMap} = this

    return (
      <MapContext.Provider
        value={{currentMap, setMap, waitingSchoolCount, counties}}
      >
        {children}
      </MapContext.Provider>
    )
